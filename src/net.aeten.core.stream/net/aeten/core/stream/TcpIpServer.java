...
public class TcpIpServer {
	...
	private TcpIpServer(InetSocketAddress destination, boolean bind, boolean reuse, int timeout, int backlog) throws IOException {
	...
	static TcpIpServer get(InetSocketAddress destination, boolean bind, boolean reuse, int timeout, int backlog) throws IOException {
	...
	public static class InputStream extends java.io.InputStream {
		...
		public InputStream(InetSocketAddress destination) throws IOException {
			this(destination, false, false, -1, -1);
		}

		public InputStream(InetSocketAddress destination, boolean bind, boolean reuse, int timeout, int backlog) throws IOException {
		...
		public int available() throws IOException {
			synchronized (server.serverSocket) {
				if (server.socket == null || server.socket.isClosed()) { return 0; }
	...
	public static class OutputStream extends java.io.OutputStream {
		...
		public OutputStream(InetSocketAddress destination) throws IOException {
			this(destination, false, false, -1, -1);
		}

		public OutputStream(InetSocketAddress destination, boolean bind, boolean reuse, int timeout, int backlog) throws IOException {
			server = TcpIpServer.get(destination, bind, reuse, timeout, backlog);
		}
